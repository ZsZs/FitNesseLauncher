...
public class CalcWikiFormatClasspathTest {
   ...
   public void testNoDependenciesNoFitNesseJarFile() throws MojoExecutionException {
      ...
      assertEquals( "\n", helper.mojo.calcWikiFormatClasspath() );
      assertEquals( format( "[WARNING] File for artifact [org.fitnesse:fitnesse:jar:20151230:compile] is not found%n" ), helper.logStream.toString() );
   ...
   private void assertProjectDependencyScopes( int artifactCount, boolean optional, String... scopes ) throws MojoExecutionException {
      ...
      String expectedLog = format( "[INFO] Using dependencies in the following scopes: %s%n%s", Arrays.toString( scopes ), (optional) ? expectedLogLine2 : "" );

      assertEquals( "\n", helper.mojo.calcWikiFormatClasspath() );
      assertEquals( expectedLog, helper.logStream.toString() );

      verify( helper.mojo.fitNesseHelper, times( 1 ) ).formatAndAppendClasspathArtifact( any( StringBuilder.class ), eq( helper.fitnesseArtifact ) );
      verify( helper.mojo.fitNesseHelper, times( artifactCount ) ).formatAndAppendClasspathArtifact( any( StringBuilder.class ), any( Artifact.class ) );
   ...
   public void testPluginDependenciesAndProjectDependencies() throws MojoExecutionException {
      ...
      when( helper.artifactResolver.resolve( argThat( new ResolutionRequestForArtifact( g1a1 ) ) ) ).thenReturn( helper.createArtifactResolutionResult( g1a1 ) );
      when( helper.artifactResolver.resolve( argThat( new ResolutionRequestForArtifact( g2a1 ) ) ) ).thenReturn( helper.createArtifactResolutionResult( g2a1 ) );

      assertEquals( "\n", helper.mojo.calcWikiFormatClasspath() );
      assertEquals( USING_PLUGIN_CONFIG + format( "[INFO] Using dependencies in the following scopes: [compile, test, runtime]%n" ), helper.logStream.toString() );

      verify( helper.mojo.fitNesseHelper, times( 1 ) ).formatAndAppendClasspathArtifact( any( StringBuilder.class ), eq( helper.fitnesseArtifact ) );
      verify( helper.mojo.fitNesseHelper, times( 1 ) ).formatAndAppendClasspathArtifact( any( StringBuilder.class ), eq( g1a1 ) );
      verify( helper.mojo.fitNesseHelper, times( 1 ) ).formatAndAppendClasspathArtifact( any( StringBuilder.class ), eq( g2a1 ) );
      verify( helper.mojo.fitNesseHelper, times( 12 ) ).formatAndAppendClasspathArtifact( any( StringBuilder.class ), any( Artifact.class ) );
